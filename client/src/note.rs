...
pub fn create(conn: &rusqlite::Connection, message: &String, foreign_id: u32, foreign_type: u8) {
    conn.execute(QUERY_CREATE,params![message,foreign_id,foreign_type]).expect("failed to insert new note");
}
